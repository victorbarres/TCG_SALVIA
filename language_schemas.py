...
class CXN_SCHEMA(KNOWLEDGE_SCHEMA):
    ...
    def SemMatch_qual(self, SemRep,
                      a_sub_iso):  ## NEEDS TO BE WRITTEN!! At this point the formalism does not support efficient quality of match.
        """
    ...
    def initialize(self, cpt_knowledge):
        ...
        for concept in cpt_knowledge.concepts():
            ...
            if concept.name == 'CONCEPT':
                ...
                print("{}: unknown concept type".format(concept.meaning))
...
class SEMANTIC_WM(WM):
    ...
    def __init__(self, name='Semantic_WM'):
        ...
        self.params['C2'] = {'coop_weight': 0.0, 'comp_weight': 0.0, 'prune_threshold': 0.01,
                             'confidence_threshold': 0.0, 'coop_asymmetry': 1.0, 'comp_asymmetry': 0.0,
        ...
        if cpt_insts:
            ...
            for inst in [i for i in cpt_insts if not (isinstance(i.trace['cpt_schema'], CPT_RELATION_SCHEMA))]:
                if self.SemRep.has_node(inst.name):
                    continue
                self.SemRep.add_node(inst.name, cpt_inst=inst, concept=inst.content['concept'], frame=inst.frame,
                                     new=True, processed=[], expressed=False)
            ...
            for rel_inst in [i for i in cpt_insts if isinstance(i.trace['cpt_schema'], CPT_RELATION_SCHEMA)]:
                ...
                if self.SemRep.has_edge(node_from, node_to):
                    continue
                self.SemRep.add_edge(node_from, node_to, name=rel_inst.name, cpt_inst=rel_inst,
                                     concept=rel_inst.content['concept'], frame=inst.frame, new=True, processed=[],
                                     expressed=False)
...
class SEMANTIC_WM_C(WM):
    ...
    def process(self):
        ...
        if gram_input:
            ...
            self.convey_gram_activations(
                gram_activations)  # NEED TO DEFINE WEIGHT IF THERE IS COMPETITION. DO THAT USING THE CONNECT WEIGHT
    ...
    def instantiate_gram_cpts(self, SemFrame, sem_map, cpt_schemas):
        ...
        def find_cpt_inst(sem_frame_insts):
            ...
            """
            output = [inst for inst in self.schema_insts if
                      not (inst.trace['sem_frame_insts'].isdisjoint(sem_frame_insts))]
            if len(output) > 1:
                error_msg = 'There is more than one cpt_inst that map onto the same SemFrame element! {}'.format(
                    str([o.name for o in output]))
        ...
        for node in SemFrame.nodes:
            ...
            if old_cpt_inst:
                ...
                new_cpt_inst = CPT_SCHEMA_INST(cpt_schema,
                                               trace={'cpt_schema': cpt_schema, 'sem_frame_insts': sem_frame_insts,
                                                      'wk_frame_insts': set([])})
    ...
    def _find_cpt_schema(self, cpt_schemas, cpt_name):
        ...
        if len(output) > 1:
            error_msg = 'There is more than one cpt_schema matches the concept name {}'.format(cpt_name)
            raise ValueError(error_msg)
        elif not (output):
            error_msg = 'There are is cpt_schema that matches the concept name {}'.format(cpt_name)
            raise ValueError(error_msg)
        else:
            return output[0]
...
class SEMANTIC_WM_C2_C(WM):
            ...
            if instance_data and cpt_schemas:  # Instantiate data from constructions
                ...
                if SemFrame and sem_map:
                    ...
                    for inst1, inst2 in competitions:
                        if self.verbose > 0:
                            print("{}, {} vs. {}".format(self.t, inst1.name, inst2.name))
                        self.add_comp_link(inst1, inst2)
            self.convey_gram_activations(gram_activations,
                                         gram_weight)  # NEED TO DEFINE WEIGHT IF THERE IS COMPETITION. DO THAT USING THE CONNECT WEIGHT
    ...
    def instantiate_gram_cpts(self, SemFrame, sem_map, cpt_schemas):
        ...
        for sub_iso in sub_isos:
            ...
            for n_s, n_t in sub_iso['nodes'].iteritems():
                ...
                if inst_s.concept.match(inst_t.content['concept'], 'is_a') and not (
                inst_s.concept.match(inst_t.content['concept'], match_type='equal')):
            ...
            for e_s, e_t_list in sub_iso['edges'].iteritems():
                dat_s = SemFrame.graph.get_edge_data(e_s[0], e_s[1],
                                                     e_s[2])  # by definition multi edges are given as (from, to, key)
                name_s = dat_s['name']
                edge_s = SemFrame.find_elem(name_s)
                sem_frame_insts = set([k for k, v in sem_map.iteritems() if name_s in v])
                names.append(name_s)
                for e_t in e_t_list:
                    dat_t = self.SemRep.get_edge_data(e_t[0], e_t[1],
                                                      e_t[2])  # by definition multi edges are given as (from, to, key)
                    name_t = dat_t['name']
                    ...
                    if edge_s.concept.match(inst_t.content['concept'], match_type='equal'):  # edge concepts match
                        ...
                        new_cpt_inst = CPT_SCHEMA_INST(cpt_schema,
                                                       trace={'cpt_schema': cpt_schema, 'wk_frame_insts': set([]),
                                                              'sem_frame_insts': set([edge_s.name])})
            ...
            for node in [n for n in SemFrame.nodes if n.name not in names]:
                cpt_schema = self._find_cpt_schema(cpt_schemas, node.concept.name)
                sem_frame_insts = set([k for k, v in sem_map.iteritems() if node.name in v])
                new_cpt_inst = CPT_SCHEMA_INST(cpt_schema, trace={'cpt_schema': cpt_schema, 'wk_frame_insts': set([]),
                                                                  'sem_frame_insts': sem_frame_insts}, frame=node.frame)
            ...
            for edge in [e for e in SemFrame.edges if e.name not in names]:
                ...
                sem_frame_insts = set([k for k, v in sem_map.iteritems() if edge.name in v])
                new_cpt_inst = CPT_SCHEMA_INST(cpt_schema, trace={'cpt_schema': cpt_schema, 'wk_frame_insts': set([]),
                                                                  'sem_frame_insts': sem_frame_insts})
        ...
        if sub_isos:
            if self.verbose > 0:
                print("{}: Add partial SemFrame".format(self.t))
            return output
        ...
        if self.verbose > 0:
            print("{}: Add full SemFrame".format(self.t))
        return output
    ...
    def instantiate_wk_cpts(self, wk_inputs, cpt_schemas):
        ...
        for wk_frame, inst_name in wk_inputs:
            sub_isos = self.FrameMatch(
                wk_frame)  # Check whether SemRep matches a subgraph of wk_frame (whether wk_frame adds information to SemRep)
            names = []
            for sub_iso in sub_isos:
                name_table = {}
                for n_s, n_t in sub_iso['nodes'].iteritems():
                    ...
                    if node_s.concept.match(inst_t.content['concept'], 'is_a') and not (
                    node_s.concept.match(inst_t.content['concept'], match_type='equal')):
                        inst_t.content['concept'] = node_s.concept  # update concept
                        inst_t.frame = node_s.frame
                        cpt_insts.append(inst_t)
                for e_s, e_t_list in sub_iso['edges'].iteritems():
                    dat_s = wk_frame.graph.get_edge_data(e_s[0], e_s[1], e_s[
                        2])  # by definition multi edges are given as (from, to, key)
                    name_s = dat_s['name']
                    edge_s = wk_frame.find_elem(name_s)
                    names.append(name_s)
                    for e_t in e_t_list:
                        dat_t = self.SemRep.get_edge_data(e_t[0], e_t[1], e_t[
                            2])  # by definition multi edges are given as (from, to, key)
                        name_t = dat_t['name']
                        inst_t = self.find_instance(name_t)
                        if edge_s.concept.match(inst_t.content['concept'], match_type='equal'):  # edge concepts match
                            ...
                            cpt_schema = self._find_cpt_schema(cpt_schemas, edge_s.concept.name)
                            new_cpt_inst = CPT_SCHEMA_INST(cpt_schema, trace={'cpt_schema': cpt_schema,
                                                                              'wk_frame_insts': set([edge_s.name]),
                                                                              'sem_frame_insts': set([])})
                            new_cpt_inst.content['pFrom'] = inst_t.content['pFrom']
                            new_cpt_inst.content['pTo'] = inst_t.content['pTo']
                            cpt_insts.append(new_cpt_inst)
                ...
                for node in [n for n in wk_frame.nodes if n.name not in names]:
                    cpt_schema = self._find_cpt_schema(cpt_schemas, node.concept.name)
                    new_cpt_inst = CPT_SCHEMA_INST(cpt_schema,
                                                   trace={'cpt_schema': cpt_schema, 'wk_frame_insts': set([node.name]),
                                                          'sem_frame_insts': set([])}, frame=node.frame)
                    name_table[node.name] = new_cpt_inst
                    cpt_insts.append(new_cpt_inst)
                    names.append(node.name)
                for edge in [e for e in wk_frame.edges if e.name not in names]:
                    cpt_schema = self._find_cpt_schema(cpt_schemas, edge.concept.name)
                    new_cpt_inst = CPT_SCHEMA_INST(cpt_schema,
                                                   trace={'cpt_schema': cpt_schema, 'wk_frame_insts': set([edge.name]),
                                                          'sem_frame_insts': set([])})
                    new_cpt_inst.content['pFrom'] = name_table[edge.pFrom.name]
                    new_cpt_inst.content['pTo'] = name_table[edge.pTo.name]
                    cpt_insts.append(new_cpt_inst)
            ...
            if sub_isos:
                if self.verbose > 0:
                    print('{}: Adding partial wk frame {}'.format(self.t, wk_frame.name))
                continue
            ...
            if self.verbose > 0:
                print("{}: Add full wk_frame {}".format(self.t, wk_frame.name))
        ...
        if cpt_insts:
            ...
            for inst in [i for i in cpt_insts if not (isinstance(i.trace['cpt_schema'], CPT_RELATION_SCHEMA))]:
                if self.SemRep.has_node(inst.name):
                    ...
                    if self.verbose > 0:
                        print("{}: Updating: {} -> {}".format(self.t, cpt1.name, cpt2.name))
                    cpt1 = cpt2
                    self.SemRep.node[inst.name]['frame'] = inst.frame

                self.SemRep.add_node(inst.name, cpt_inst=inst, concept=inst.content['concept'], frame=inst.frame,
                                     new=True, processed=[], expressed=False, dat=(inst.content['concept'], inst.frame))
            ...
            for rel_inst in [i for i in cpt_insts if isinstance(i.trace['cpt_schema'], CPT_RELATION_SCHEMA)]:
                ...
                node_to = rel_inst.content['pTo'].name
                self.SemRep.add_edge(node_from, node_to, name=rel_inst.name, cpt_inst=rel_inst,
                                     concept=rel_inst.content['concept'], frame=rel_inst.frame, new=True, processed=[],
                                     expressed=False, dat=(rel_inst.content['concept'], rel_inst.frame))
    ...
    def FrameMatch_cat(self, frame_graph):
        ...
        node_concept_match = lambda cpt1, cpt2: cpt2.match(cpt1, match_type="is_a") or cpt1.match(cpt2,
                                                                                                  match_type="is_a")
        edge_concept_match = lambda cpt1, cpt2: cpt1.match(cpt2, match_type="is_a")  # "equal" for strict matching
        ...
        sub_isos = TCG_graph.find_sub_multi_iso(self.SemRep, frame_graph, node_match=nm, edge_match=em,
                                                induce_type='edge')
        ...
        sub_isos = TCG_graph.find_sub_multi_iso(self.SemRep, frame_graph, node_match=nm, edge_match=None,
                                                induce_type='edge')
        # Enforcing co-reference
        filtered_sub_isos = [s for s in sub_isos if iso_coref_filter(s, refs_s, refs_t, corefs)]
        if filtered_sub_isos:
        ...
        sub_isos = TCG_graph.find_max_partial_iso(self.SemRep, frame_graph, node_match=nm, edge_match=em,
        ...
        sub_isos = TCG_graph.find_max_partial_iso(self.SemRep, frame_graph, node_match=nm, edge_match=None,
                                                  target_induce_type='edge', source_induce_type='edge')
        # Enforcing co-reference
        filtered_sub_isos = [s for s in sub_isos if iso_coref_filter(s, refs_s, refs_t, corefs)]
        if filtered_sub_isos:
...
class GRAMMATICAL_WM_P(WM):
    ...
    def __init__(self, name='Grammatical_WM_P'):
        ...
        self.add_port('OUT', 'to_output')
        self.params['dyn'] = {'tau': 30.0, 'int_weight': 1.0, 'ext_weight': 1.0, 'act_rest': 0.001, 'k': 10.0,
                              'noise_mean': 0.0, 'noise_std': 0.3}
        self.params['C2'] = {'coop_weight': 1.0, 'comp_weight': -4.0, 'coop_asymmetry': 1.0, 'comp_asymmetry': 0.0,
                             'max_capacity': None, 'P_comp': 1.0, 'P_coop': 1.0, 'deact_weight': 0.0,
                             'prune_threshold': 0.3, 'confidence_threshold': 0.8, 'sub_threshold_r': 0.8,
                             'refractory_period': 10}
        self.params['style'] = {'activation': 1.0, 'sem_length': 0, 'form_length': 0,
                                'continuity': 0}  # Default value, updated by control.
    ...
    def process(self):
        """
        """
        sem_input = self.inputs[
            'from_semantic_WM']  # I need to tie the activity of the cxn_instances to that of the SemRep. (Would be nicer to have coop-links across WMs.)
    ...
    def convey_sem_activations(self, sem_input):
        ...
        for inst in self.schema_insts:
            ...
            for node in sem_input['nodes']:
                inst_node = next((k for k, v in cover_nodes.items() if v == node),
                                 None)  # Instances covers the node through sf_node
                if inst_node:
                    inst_form = inst.content.node2form(inst_node)
                    if isinstance(inst_form,
                                  construction.TP_PHON):  # sf_node is linked to a TP_PHON form or does not have a symlink.. (formalization)
            ...
            for edge in sem_input['edges']:
                ...
                if inst_edge:
                    act_edge += sem_input['edges'][
                        edge]  # Edge always propagate their activation since they are obligatory formalized in a TCG cxn.
                    count_edge += 1
            ...
            if new_inst != old_inst:
                ...
                if match["match_cat"] == 1:
                    for match_qual, link in match["links"]:
                        if match_qual > 0:
                            self.add_coop_link(inst_from=link["inst_from"], port_from=link["port_from"],
                                               inst_to=link["inst_to"], port_to=link["port_to"], qual=match_qual)
    ...
    def comp_link(inst_1, inst_2, SR_node):
        ...
        sf_1 = [cxn_1.find_elem(k) for k, v in inst_1.covers["nodes"].iteritems() if v == SR_node][
            0]  # Find SemFrame node that covers the SemRep node
        cxn_2 = inst_2.content
        sf_2 = [cxn_2.find_elem(k) for k, v in inst_2.covers["nodes"].iteritems() if v == SR_node][
            0]  # Find SemFrame node that covers the SemRep node

        cond1 = (sf_1.name not in cxn_1.SymLinks.SL) or isinstance(cxn_1.node2form(sf_1),
                                                                   construction.TP_PHON)  # cxn_1 formalizes the node entity
        cond2 = (sf_2.name not in cxn_2.SymLinks.SL) or isinstance(cxn_2.node2form(sf_2),
                                                                   construction.TP_PHON)  # cxn_2 formalizes the node entity
    ...
    def coop_link(inst_p, inst_c, SR_node):
        ...
        sf_p = [cxn_p.find_elem(k) for k, v in inst_p.covers["nodes"].iteritems() if v == SR_node][
            0]  # Find SemFrame node that covers the SemRep node
        cxn_c = inst_c.content
        sf_c = [cxn_c.find_elem(k) for k, v in inst_c.covers["nodes"].iteritems() if v == SR_node][
            0]  # Find SemFrame node that covers the SemRep node

        # Type constraints (Obligatory)
        syn1 = (sf_p.name in cxn_p.SymLinks.SL) and isinstance(cxn_p.node2form(sf_p),
                                                               construction.TP_SLOT)  # sf_p is linked to a slot in cxn_p
        ...
        if syn1 and sem1:
            ...
            link = {"inst_from": inst_c, "port_from": inst_c.find_port("output"), "inst_to": inst_p,
                    "port_to": inst_p.find_port(slot_p.order)}
    ...
    def match(inst1, inst2):
        ...
        if flag1 and flag2:
            print("LOOP {} {}".format(inst1.name, inst2.name))  # Warns that there are direct loops.
       ...
       Notes: 
        ...
        if assemblages:
            ...
            if winner_dat:  # LOOK INTO THIS!!! THIS IS AN IMPORTANT STEP
                ...
                data.append({'t': self.t, 'assemblage': winner_assemblage.copy(), 'phon_form': phon_form[:],
                             'eq_inst': eq_inst.content.copy()[0], 'a2i_map': a2i_map.copy(),
                             'expressed': expressed.copy(), 'partial_readout': partial_readout,
                             'insts_used': insts_used})
    ...
    def get_winner_assemblage(self, assemblages, sem_input, phon_input):
        ...
        assemblages_dat = []
        for assemblage in assemblages:
            (phon_form, missing_info, expressed, eq_inst, a2i_map, insts_used) = GRAMMATICAL_WM_P.form_read_out(
                assemblage)  # In order to test for continuity, I have to read_out every assemblage.
            assemblages_dat.append((assemblage, phon_form, missing_info, expressed, eq_inst, a2i_map, insts_used))
            sem_length_nodes = len([sf_node for sf_node, semrep_node in eq_inst.covers['nodes'].iteritems() if (
                        semrep_node in sem_input['nodes'])])  # Only counts nodes that have NOT already been expressed.
            sem_length_edges = len([sf_edge for sf_edge, semrep_edge in eq_inst.covers['edges'].iteritems() if
                                    semrep_edge in sem_input[
                                        'edges']])  # Only counts edges that have NOT already been expressed.
        ...
        for i in range(len(assemblages)):
            score = w1 * assemblages[i].activation + w2 * scores['sem_length'][i] + w3 * (
                        1 - scores['form_length'][i]) + w4 * scores['continuity'][i]  # Scoring assemblage
    ...
    def get_trees(self, frontier, assemblage, graph, results):
        ...
        """
        new_frontiers = [
            []]  # Each frontier correspond to a possible choice between multiple cooperation options to a same port.
        ...
        for node, link in frontier:
            ...
            for port in ports:
                children = graph.predecessors(
                    port)  # A port can be linked to multiple children, each representing a different hypothesis.
                updated_frontiers = []
                if not (children):
                    pass
                else:
                    for child in children:
                        ...
                        if not (flag):
                            link = self.find_coop_links(inst_from=child, inst_to=node,
                                                        port_from=child.find_port("output"), port_to=port)
    ...
    def assemblage2inst(assemblage):
        ...
        if coop_links:  # not a trivial assemblage composed of a single instance.
            while len(coop_links) > 0:
                (new_assemblage, new_cxn_inst, a2i_map) = GRAMMATICAL_WM_P.reduce_assemblage(new_assemblage,
                                                                                             new_assemblage.coop_links[
                                                                                                 0], a2i_map)
                coop_links = new_assemblage.coop_links
            ...
            a2i_map['sem_map'] = dict([(sem_elem.name, sem_elem.name) for sem_elem in
                                       inst.content.SemFrame.nodes + inst.content.SemFrame.edges])
    ...
    def combine_schemas(inst_to, inst_from, connect, a2i_map):
        ...
        # Update a2i_map
        for map_type in ['sem_map',
                         'syn_map']:  # There must be a cleaner way to do that! Rethink the data structure used.
            new = {}
            to_remove = set([])
    ...
    def form_read_out(assemblage):
        ...
        if not (missing_info):  # Everything has been expressed.
            expressed = {'nodes': eq_inst.trace['semrep']['nodes'][:],
                         'edges': eq_inst.trace['semrep']['edges'][:]}  # Deep copy
    ...
    def draw_instance_network(graph, title=''):
        ...
        nx.draw_networkx_nodes(graph, pos, nodelist=[n for n in graph.nodes() if graph.node[n]['type'] == 'instance'],
                               node_color='b', node_shape='s', node_size=300)
        nx.draw_networkx_nodes(graph, pos, nodelist=[n for n in graph.nodes() if graph.node[n]['type'] == 'port'],
                               node_color='r', node_shape='h', node_size=200)
        nx.draw_networkx_edges(graph, pos=pos,
                               edgelist=[e for e in graph.edges() if graph.edge[e[0]][e[1]]['type'] == 'port2inst'],
                               edge_color='k')
        nx.draw_networkx_edges(graph, pos=pos,
                               edgelist=[e for e in graph.edges() if graph.edge[e[0]][e[1]]['type'] == 'inst2port'],
                               edge_color='r')
    ...
    def instantiate_cxns(self, SemRep, cxn_schemas):
        ...
        for cxn_schema in cxn_schemas:
            ...
            for a_sub_iso, match_qual in match_output:
                trace = {"semrep": {"nodes": a_sub_iso["nodes"].values(), "edges": a_sub_iso["edges"].values()},
                         "schemas": [cxn_schema]}
...
class PHON_WM_P(WM):
    ...
    def __init__(self, name='Phonological_WM_P'):
        ...
        self.add_port('OUT', 'to_control')
        self.add_port('OUT', 'to_output')
        self.params['dyn'] = {'tau': 2, 'int_weight': 1.0, 'ext_weight': 1.0, 'act_rest': 0.001, 'k': 10.0,
                              'noise_mean': 0.0, 'noise_std': 0.0}
        self.params['C2'] = {'coop_weight': 0.0, 'comp_weight': 0.0, 'prune_threshold': 0.01,
                             'confidence_threshold': 0.0, 'coop_asymmetry': 1.0, 'comp_asymmetry': 0.0,
                             'max_capacity': None, 'P_comp': 1.0, 'P_coop': 1.0}  # C2 is not implemented in this WM.
...
class GRAMMATICAL_WM_C(WM):
    ...
    def __init__(self, name='Grammatical_WM_C'):
        ...
        self.params['C2'] = {'coop_weight': 1.0, 'comp_weight': -4.0, 'coop_asymmetry': 1.0, 'comp_asymmetry': 0.0,
                             'max_capacity': None, 'P_comp': 1.0, 'P_coop': 1.0, 'deact_weight': 0.0,
                             'prune_threshold': 0.3, 'confidence_threshold': 0.8, 'sub_threshold_r': 0.8}
        self.params['parser'] = {'pred_init': {'S': [1]},
                                 'parser_type': 'Left-Corner'}  # S is used to initialize the set of predictions. This is not not really in line with usage based... but for now I'll keep it this way.
        self.state = -1
        self.pred_init = None
    ...
    def process(self):
        ...
        if self.params['parser']['parser_type'] == 'Earley':
            print(
                "Sorry! Earley parser has been removed in this version. Use Left-Corner. But top-down filtering will come back soon!")
        ...
        while completed_insts:
            ...
            for inst1 in completed_insts:
                ...
                for inst2 in incomplete_insts:
                    coop = self.cooperate(inst2,
                                          inst1)  # Attempt cooperation. If possible, both create coop_link AND update instance states.
                    if coop:
    ...
    def link(inst_p, inst_c):
        ...
        if cond1:
            ...
            cond3 = sem_elem_c.concept.match(sem_elem_p.concept,
                                             match_type="is_a")  # CHECK LIGHT_SEM MATCH. NEED TO HAVE QUALITATIVE MATCH (NOT BOOLEAN MATCH)

            link = {"inst_from": inst_c, "port_from": inst_c.find_port("output"), "inst_to": inst_p,
                    "port_to": inst_p.find_port(form_elem_p.order)}
            if cond2 and cond3:
                match_qual = 1
            else:
                match_qual = 0
    ...
    def produce_meaning(self):
        ...
        """
        assemblages = self.assemble()
        is_expressed = lambda assemblage: [i for i in assemblage.schema_insts if not (
            i.expressed)] == []  # Returns true if assemblage only contains instances that have already been expressed.
        ...
        if assemblages:
            ...
            has_new_info = not is_expressed(winner_assemblage)
            if winner_assemblage.activation > self.params['C2'][
                'confidence_threshold'] and has_new_info:  # only bother if the winner assemblage is above threshold and contains new info
                print("USING FORM_MEANING MAPPING: {}".format(eq_inst.name))
    ...
    def assemblage2inst(assemblage):
        ...
        if coop_links:  # not a trivial assemblage composed of a single instance.
            while len(coop_links) > 0:
                (new_assemblage, new_cxn_inst, a2i_map) = GRAMMATICAL_WM_C.reduce_assemblage(new_assemblage,
                                                                                             new_assemblage.coop_links[
                                                                                                 0], a2i_map)
                coop_links = new_assemblage.coop_links
            eq_inst = new_assemblage.schema_insts[0]
    ...
    def process(self):
        ...
        if self.state['mode'] == 'produce':
            ...
            if self.t == self.params['task']['start_produce']:
                self.state[
                    'last_prod_time'] = self.t  # pressure only starts building up once the start_produce time has been reached.
            ...
            if self.t < self.params['task']['start_produce'] or not (self.state['unexpressed_sem']):
                pressure = 0
                self.state[
                    'last_prod_time'] = self.t  # This option allows to have the pressure start to ramp up only once a new sem element has been introduced in semWM.
            else:
                pressure = min((self.t - self.state['last_prod_time']) / self.params['task']['time_pressure'],
                               1)  # Pressure ramps up linearly to 1

            output = {'produce': self.state['produce'], 'pressure': pressure,
                      'params_style': self.params['style'].copy()}
...
class ISRF_INTERPRETER(object):
    ...
    def prop_interpreter(self, proposition):
        ...
        for term in proposition:
            ...
            if match1:
                ...
                error_msg = "Unknown formula {}".format(term)
                raise ValueError(error_msg)
        ...
        for match in cpts_match:  # First process defined concept schemas
            ...
            cpt_inst = CPT_SCHEMA_INST(cpt_schema, trace={'per_inst': None, 'cpt_schema': cpt_schema,
                                                          'ref': var})  # 'ref' is used to track referent.
        ...
        for match in rels_match:  # Then build the relations
            ...
            if not ((rel in self.name_table) and (arg1 in self.name_table) and (arg2 in self.name_table)):
                error_msg = "ISRF variable used before it is defined. {}({}, {})".format(rel, arg1, arg2)
                raise ValueError(error_msg)
            else:
    ...
    def prop_grapher(proposition):
        ...
        for var in [v for v in name_table if v not in rel_table]:  # First process cpt nodes
            graph.add_node(var, concept=name_table[var]['concept'], act=name_table[var]['act'],
                           frame=name_table[var]['frame'])
...
class ISRF_WRITER(object):
    ...
    def cpt_2_ISRF(self, cpt_inst):
        ...
        if not var_name:
            var_name = "{}_{}".format(str.lower(cpt_name), self.var_id)
            self.var_table[cpt_inst.name] = var_name
            self.var_id += 1
        cpt_ISRF = "{}({}, ".format(cpt_name, var_name)
        if cpt_inst.frame:
            cpt_ISRF += "F, "
        cpt_ISRF += "{:.2f})".format(cpt_inst.activity)
    ...
    def write_ISRF(self):
        ...
        for cpt_inst in [i for i in cpt_insts if
                         not (isinstance(i.trace['cpt_schema'], CPT_RELATION_SCHEMA))]:  # Start with nodes.
            (cpt_ISRF, var_name) = self.cpt_2_ISRF(cpt_inst)
            dat.append(cpt_ISRF)
        for rel_inst in [i for i in cpt_insts if
                         isinstance(i.trace['cpt_schema'], CPT_RELATION_SCHEMA)]:  # Then move on to relations
            (cpt_ISRF, var_name) = self.cpt_2_ISRF(rel_inst)
            dat.append(cpt_ISRF)
            p_from = rel_inst.content['pFrom']
            p_to = rel_inst.content['pTo']
            cpt_ISRF_rel = "{}({}, {})".format(var_name, self.var_table[p_from.name], self.var_table[p_to.name])
            dat.append(cpt_ISRF_rel)
        s = ' {} '.format(self.connector)
        dat = s.join(dat)
        self.data[t] = dat
...
class SEM_GENERATOR(object):
        ...
        for name, sem_input in self.sem_inputs.iteritems():
            ...
            if sem_rate and not (timing):
                if sem_rate < 2 * self.std:
                    error_msg = "Input sequence order compromised. Sem_Rate={:.2f} < 2*std={:.2f}. SemRate should be > 2*std".format(
                        sem_rate, 2 * self.std)
                    raise ValueError(error_msg)
                sem_input['timing'] = [get_time(i, sem_rate, self.std) + self.offset for i in range(len(sequence))]
            if not (timing) and not (sem_rate):
                print("PREPROCESSING ERROR: Provide either timing or rate for {}".format(name))
    ...
    def show_input(self, input_name):
        ...
        if sem_input:
            ...
            for i in range(len(sequence)):
                print('t: {:.1f}, prop: {}'.format(timing[i], str(propositions[sequence[i]])))
    ...
    def sem_generator(self, input_name, verbose=False):
        ...
        for idx in range(len(sequence)):
            ...
            if verbose:
                print('sem_input <- t: {:.1f}, prop: {}'.format(timing[idx], str(proposition)))
...
class UTTER_GENERATOR(object):
    ...
    def show_input(self, input_name):
        ...
        if ling_input:
            ...
            for i in range(len(utterance)):
                print('t: {:.1f}, word-form: {}'.format(timing[i], utterance[i]))
    ...
    def utter_generator(self, input_name, verbose=False):
        ...
        for idx in range(len(utterance)):
            ...
            if verbose:
                print('ling_input <- t: {:.1f}, word_form: {}'.format(timing[idx], word_form))
