...
def set_inputs(model, input_name, sem_input_file='diagnostic.json', sem_input_macro=False, speed_param=10, offset=0,
               std=0):
    """
    ...
    if not (sem_input_macro):
        ...
        if input_name == 'ALL':
            sem_gen = ls.SEM_GENERATOR(sem_inputs, conceptLTM, speed_param=speed_param, offset=offset, std=std,
                                       is_macro=sem_input_macro)
            ...
            sem_gen = ls.SEM_GENERATOR(sem_input, conceptLTM, speed_param=speed_param, offset=offset, std=std,
                                       is_macro=sem_input_macro)
...
def run(model, sem_gen, input_name, sim_name='', sim_folder=TMP_FOLDER, max_time=900, seed=None, verbose=0,
        prob_times=[], save=False, anim=False, anim_step=10, all_imgs=False):
    """
    ...
    for t in range(max_time):
        ...
        if output['Grammatical_WM_P'] and output['Grammatical_WM_P'][0]['phon_form']:
            if verbose > 1:
                print("t:{}, '{}'".format(t, ' '.join(output['Grammatical_WM_P'][0]['phon_form'])))
            if verbose > 1:
        ...
        if t in prob_times or all_imgs:  # Saving figures for prob times.
            TCG_VIEWER.display_lingWM_state(model.schemas['Semantic_WM_P'], model.schemas['Grammatical_WM_P'],
                                            concise=True, folder=FOLDER, file_type='png', show=False)
            TCG_VIEWER.display_gramWM_state(model.schemas['Grammatical_WM_P'], concise=True, folder=FOLDER,
...
def get_requested_info(outputs):
    ...
    for t in sorted(outputs):
        ...
        if r:
            request = '<{}>{}({})'.format(t, r['missing_info'], r['var_name'])
...
def run_model(semantics_name='TCG_semantics_main', grammar_name='TCG_grammar_VB_main', sim_name='',
              sim_folder=TMP_FOLDER, model_params={}, input_name='woman_kick_man_static',
              sem_input_file='diagnostic.json', sem_input_macro=False, max_time=900, seed=None, speed_param=10,
              offset=0, std=0, prob_times=[], verbose=0, save=True, anim=False, anim_step=10):
    """
    Runs the model
    ...
    if sem_gen.is_macro:
        for name in sem_gen.sem_inputs:
            out[name] = run(model, sem_gen, name, sim_name=sim_name, sim_folder=sim_folder, max_time=max_time,
                            seed=seed, verbose=verbose, prob_times=prob_times, save=save, anim=anim,
                            anim_step=anim_step)
    else:
        out[input_name] = run(model, sem_gen, input_name, sim_name=sim_name, sim_folder=sim_folder, max_time=max_time,
                              seed=seed, verbose=verbose, prob_times=prob_times, save=save, anim=anim,
                              anim_step=anim_step)
    ...
    if verbose > 0:
        ...
        for k, v in out.iteritems():
            ...
            out2str += '{}\n'.format(info)
            out2str += 'analysis:\n'
            out2str += '{}\n\n'.format(json.dumps(summarize_data(v, ground_truth), sort_keys=True, indent=4))
            out2str += '#############'
        print(out2str)
    return out, out2str
...
def run_diagnostics(verbose=3, prob_times=[]):
    ...
    model = set_model(semantics_name, grammar_name, model_params=MODEL_PARAMS)
    my_inputs = set_inputs(model, 'ALL', sem_input_file=DIAGNOSTIC_FILE, sem_input_macro=SEM_MACRO,
                           speed_param=SPEED_PARAM, offset=OFFSET, std=STD)
    ...
    for input_name in diagnostic_cases:
        print("\nINPUT NAME: {}\n".format(input_name))
        print("\nSIMULATION RUN:\n")
        res = run(model, my_inputs, input_name, sim_name='', sim_folder=TMP_FOLDER, max_time=max_time, seed=seed,
...
def parameter_space(folder=None, input_rate=100):
    ...
    max_capacity = [None]
    prune_thresholds = [
        0.0]  # Change prune threshold (should be done in relation to initial activation values.) #0.01 # Manipulations can yield "broca's aphasia" (0.3)
    conf_tresholds = [0.3]  # np.linspace(0.1, 0.9, 2) # np.linspace(0.3,0.3, 1) #0.7
    sub_threholds = [0.8]
    deact_weights = [0.0]  # np.linspace(0.0, 0.9, 2)

    # Dyn parameters
    taus = np.linspace(INPUT_RATE / 3, INPUT_RATE * 3,
                       3)  # Need to analyze the impact of that factor with respect to the rates of input to other WM and their own tau.
    ...
    param_iter = itertools.product(start_produces, time_pressures, coop_weights, coop_asymmetries, comp_weights,
                                   max_capacity, prune_thresholds, conf_tresholds, sub_threholds, deact_weights, taus,
                                   ks, act_rests, noise_stds, ext_weights)
...
def grid_search(model, sem_gen, input_name, max_time, folder, model_params_set=[], num_restarts=10, seed=None,
                verbose=2, save_models=True):
    """
    ...
    for model_params in model_params_set:
        ...
        for name in sem_gen.sem_inputs:
            ...
            for i in range(num_restarts):
                ...
                sim_output = run(model, sem_gen, name, sim_name=sim_name, sim_folder=folder, max_time=max_time,
                                 seed=seed, verbose=verbose, prob_times=[], save=save_models)
                # Summerize output
...
def run_grid_search(sim_name='', sim_folder=TMP_FOLDER, seed=None, save=True, intermediate_save=True, speak=True):
    ...
    meta_params = {'seed': seed, 'num_restarts': NUM_RESTARTS, 'input_rate': INPUT_RATE, 'input_offset': INPUT_OFFSET,
                   'input_std': INPUT_STD, 'max_time': max_time}
    ...
    for name in inputs:
        ...
        sem_gen = set_inputs(model, input_name, sem_input_file, sem_input_macro, speed_param=INPUT_RATE,
                             offset=INPUT_OFFSET, std=INPUT_STD)
        st_save(sem_gen, 'sem_gen_' + input_name, folder)

        grid_output = grid_search(model=model, sem_gen=sem_gen, input_name=input_name, max_time=max_time, folder=folder,
                                  model_params_set=model_params_set, num_restarts=NUM_RESTARTS, seed=seed,
                                  verbose=verbose, save_models=False)
...
def grid_search_to_csv(grid_output, folder, input_name, meta_params, model_params_set, param_name_mapping):
    ...
    header = [param_name_mapping.get(name, name) for name in
              param_names] + gram_output_names + sem_output_names + phon_output_names
...
#    file_name = './{}/output.txt'.format(TMP_FOLDER)
#    with open(file_name, 'w') as f:
#        f.write(out2str)
