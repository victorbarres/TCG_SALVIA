...
def set_inputs(model, ling_input_file='ling_inputs_2route.json', input_name='ALL', speed_param=10, offset=10, std=0):
    ...
    if input_name != 'ALL':
        if input_name not in ling_inputs:
            error_msg = "{} input cannot be found in {}".format(input_name, ling_input_file)
            raise ValueError(error_msg)
        ling_inputs = {input_name: ling_inputs[input_name]}
        if not (ground_truths) or input_name not in ground_truths:
            warn_msg = "No ground truth for {} in {}".format(input_name, ling_input_file)
            warnings.warn(warn_msg)
        ground_truths = ground_truths.get(input_name, None)
    else:
        if not (ground_truths):
            warn_msg = "No ground truth in {}" % ling_input_file
            warnings.warn(warn_msg)
...
def run(model, utter_gen, input_name, sim_name='', sim_folder=TMP_FOLDER, max_time=900, seed=None, verbose=0,
        prob_times=[], save=False, anim=False, anim_step=1, all_imgs=False):
    """
    ...
    for t in range(max_time):
        ...
        if test_not_empty(output):  # filter out ouputs with all valyues == None
            outputs[t] = output
            # Convert to ISRF
            change_sem_state = outputs[t]['Semantic_WM_C']
        ...
        if t in prob_times or all_imgs:  # Saving figures for prob times (or for all time if all_imgs is True).
...
def summarize_data(outputs, ground_truths=None):
    ...
    # SemWM
    sem_summary = analysis_sem(final_output['Semantic_WM_C'],
                               ground_truths)  # Should indicate whether or not proper TRA was achieved

    summary = {'GramWM': gram_summary, 'WkWM': wk_summary, 'SemWM': sem_summary}
    return summary


def run_model(semantics_name='TCG_semantics_main', grammar_name='TCG_grammar_VB_2routes', sim_name='',
              sim_folder=TMP_FOLDER, model_params={}, input_name='woman', ling_input_file='ling_inputs_2routes.json',
              max_time=900, seed=None, speed_param=10, offset=10, std=0, prob_times=[], verbose=0, save=True,
              anim=False, anim_step=10):
    """
    Runs the model
    ...
    out = {}
    out[input_name] = run(model, utter_gen, input_name, sim_name=sim_name, sim_folder=sim_folder, max_time=max_time,
                          seed=seed, verbose=verbose, prob_times=prob_times, save=save, anim=anim, anim_step=anim_step)
    return out
...
def run_diagnostic(verbose=3):
    ...
    utter_gen = set_inputs(model, LING_INPUT_FILE, input_name, speed_param=SPEED_PARAM, offset=INPUT_OFFSET,
                           std=INPUT_STD)
    res = run(model, utter_gen, input_name, sim_name='', sim_folder=TMP_FOLDER, max_time=MAX_TIME, seed=SEED,
              verbose=VERBOSE, prob_times=PROB_TIMES, save=SAVE, anim=ANIM, anim_step=ANIM_STEP)
...
def parameter_space(folder=None, input_rate=100):
    ...
    coop_asymmetries_G = [0.0]  # THIS IS AN IMPORTANT REQUIREMENT OF THIS MODEL!
    comp_weights_G = [-1.0]
    max_capacity_G = [None]
    ...
    # Dyn parameters
    taus_G = [
        100]  # [INPUT_RATE] #[INPUT_RATE/10, INPUT_RATE*10] #np.linspace(int(INPUT_RATE/10), INPUT_RATE*10, 3) # Need to analyze the impact of that factor with respect to the rates of input to other WM and their own tau.
    ks_G = [10]  # np.linspace(1, 10, 2)
    act_rests_G = [0.001]  # Act rest does not take into account the noise.
    noise_stds_G = [
        0.3]  # np.linspace(1.0, 2.0, 2) # Impact of dynamic noise -> Not useful. But might have impact in early symmetry breaking.
    ...
    max_capacity_S = [None]
    prune_thresholds_S = [
        0.0]  # Change prune threshold (should be done in relation to initial activation values.) #0.01
    conf_tresholds_S = [0.5]  # np.linspace(0.1, 0.9, 2) # np.linspace(0.3,0.3, 1) #0.7
    ...
    noise_stds_S = [
        1.0]  # np.linspace(1.0, 2.0, 2) # Impact of dynamic noise -> Not useful. But might have impact in early symmetry breaking.
    ...
    prune_thresholds_WK = [
        0.01]  # Change prune threshold (should be done in relation to initial activation values.) #0.01
    ...
    param_iter = itertools.product(coop_weights_G, coop_asymmetries_G, comp_weights_G,
                                   ...
                                   comp_weights_WK, max_capacity_WK, prune_thresholds_WK, conf_tresholds_WK,
    ...
    for param in param_iter:
        params = {'Grammatical_WM_C.C2.coop_weight': param[0],
                  ...
                  'Grammatical_WM_C.dyn.tau': param[6],
                  # Need to analyze the impact of that factor with respect to the rates of input to other WM and their own tau.
                  ...
                  'Semantic_WM_C.dyn.tau': param[15],
                  # Need to analyze the impact of that factor with respect to the rates of input to other WM and their own tau.
                  ...
                  'WK_frame_WM.C2.comp_weight': param[20],
                  'WK_frame_WM.C2.max_capacity': param[21],
                  'WK_frame_WM.C2.prune_threshold': param[22],
                  'WK_frame_WM.C2.confidence_threshold': param[23],
                  'WK_frame_WM.dyn.tau': param[24],
                  # Need to analyze the impact of that factor with respect to the rates of input to other WM and their own tau.
                  'WK_frame_WM.dyn.k': param[25],
                  'WK_frame_WM.dyn.act_rest': param[26],
                  'WK_frame_WM.dyn.noise_std': param[27],
                  'WK_frame_WM.dyn.ext_weight': param[28]}
    ...
    param_name_mapping = {'Grammatical_WM_C.C2.coop_weight': 'coop_weight_G',
                          ...
                          'Semantic_WM_C.C2.comp_weight': 'comp_weight_S',
                          ...
                          'WK_frame_WM.C2.comp_weight': 'comp_weight_WK',
...
def grid_search(model, utter_gen, input_name, max_time, folder, model_params_set=[], model_weights_set=[],
                num_restarts=10, seed=None, verbose=0, save_models=True):
    """
        ...
        for weight_params in model_weights_set:
            ...
            for name in utter_gen.ling_inputs:
                ...
                for i in range(num_restarts):
                    start = time.time()
                    sim_name = '{}_{}'.format(input_name, name)
                    sim_output = run(model, utter_gen, name, sim_name=sim_name, sim_folder=folder, max_time=max_time,
                                     seed=seed, verbose=verbose, prob_times=[], save=save_models, anim=False,
                                     anim_step=10)
                    # Summerize output
                    ...
                    if verbose > 0:
                        print("RUN {} OF {} ({:.2f}s) (remaining {})".format(count, num_sim, sim_time, remaining_time))
                    count += 1
    ...
    if verbose > 0:
        print("TOTAL GRID SEARCH TIME: {}".format(grid_time))
    return grid_output
...
def run_grid_search(sim_name='', sim_folder=TMP_FOLDER, seed=None, save=True, intermediate_save=True, speak=True):
    ...
    if not (sim_name):
        sim_name = '{}_({})'.format(sim_time, str(seed))
    else:
        sim_name = '{}_{}_({})'.format(sim_time, sim_name, str(seed))
    ...
    max_time = 1000  # 10*INPUT_RATE

    # Saving meta parameters
    meta_params = {'seed': seed, 'num_restarts': NUM_RESTARTS, 'input_rate': INPUT_RATE, 'input_std': INPUT_STD,
                   'max_time': max_time}
    ...
    test_input = [u'reversible_animate(act)', u'reversible_animate(pas)']
    ...
    for name in inputs:
        input_name = name
        print("\nProcessing input: {} ({}/{})".format(input_name, count, len(inputs)))
        ...
        grid_output = grid_search(model=model, utter_gen=utter_gen, input_name=input_name, max_time=max_time,
                                  folder=folder, model_params_set=model_params_set, model_weights_set=model_weights_set,
                                  num_restarts=NUM_RESTARTS, seed=seed, verbose=verbose, save_models=False)
        ...
        if speak:
            ls.tell_me('Grid search {} done. {} remaining.'.format(count, len(inputs) - count))
        count += 1
    ...
    print("TOTAL SIMULATION TIME: {}".format(sim_time))
...
def grid_search_to_csv(grid_output, folder, input_name, meta_params, model_params_set, param_name_mapping,
    ...
    header = [name_mapping.get(name, name) for name in
              param_names] + gram_output_names + wk_output_names + sem_output_names
    line = lambda vals: ','.join([str(v) for v in vals]) + '\n'

    file_name = './{}/{}.csv'.format(folder, input_name)
