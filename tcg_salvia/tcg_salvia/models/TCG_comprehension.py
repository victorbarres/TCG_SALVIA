...
from model_def import TCG_comprehension_system
...
def run(model, utter_gen, input_name, sim_name='', sim_folder=TMP_FOLDER, max_time=900, seed=None, verbose=0, prob_times=[], save=False, anim=False, anim_step=10):
    ...
    for t in range(max_time):
        ...
        if t in prob_times: # Saving figures for prob times.
            VIEWER.display_gramWM_state(model.schemas['Grammatical_WM_C'], concise=True, folder = FOLDER)
            VIEWER.display_semWM_state(model.schemas['Semantic_WM_C'], folder = FOLDER)
