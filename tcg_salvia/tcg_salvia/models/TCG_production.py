...
def set_inputs(model, input_name, sem_input_file='diagnostic.json', sem_input_macro=False, speed_param=10, std=0):
    ...
    if not(sem_input_macro):
        sem_inputs = LOADER.load_sem_input(sem_input_file, SEM_INPUT_PATH)
        if input_name == 'ALL':
            sem_gen = ls.SEM_GENERATOR(sem_inputs, conceptLTM, speed_param=speed_param, std=std, is_macro=sem_input_macro)
            sem_gen.ground_truths = LOADER.load_ground_truths(sem_input_file, SEM_INPUT_PATH)
        else:
            ...
            ground_truths = LOADER.load_ground_truths(sem_input_file, SEM_INPUT_PATH)
            sem_gen.ground_truths = ground_truths.get(input_name, None)
    if sem_input_macro:
        sem_inputs = LOADER.load_sem_macro(input_name, sem_input_file, SEM_INPUT_PATH)
        sem_gen = ls.SEM_GENERATOR(sem_inputs, conceptLTM, speed_param=speed_param, std=std, is_macro=sem_input_macro)
        ground_truths = LOADER.load_ground_truths(sem_input_file, SEM_INPUT_PATH)
        sem_gen.ground_truths = ground_truths.get(input_name, None)
...
def run(model, sem_gen, input_name, sim_name='', sim_folder=TMP_FOLDER, max_time=900, seed=None, verbose=0, prob_times=[], save=False, anim=False, anim_step=10):
    ...
    for t in range(max_time):
        ...
        if t in prob_times: # Saving figures for prob times.
            VIEWER.display_lingWM_state(model.schemas['Semantic_WM_P'], model.schemas['Grammatical_WM_P'], concise=True, folder = FOLDER)
