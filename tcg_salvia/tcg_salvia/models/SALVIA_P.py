...
from model_def import SALVIA_P, SALVIA_P_verbal_guidance
...
def set_inputs(model, input_name, input_file='TCG_scene.json', show_scene=True):
    ...
    my_scene = LOADER.load_scene(input_file, SCENE_FOLDER, perceptLTM)
            ...
            if output['Subscene_recognition']:
                ...
                if eye_pos:
                    ...
                    if verbose>1:
                        ...
                        print "t:%i, '%s'" %(t, vals)
        if t in prob_times:
                VIEWER.display_WMs_state(model.schemas['Visual_WM'], model.schemas['Semantic_WM_P'], model.schemas['Grammatical_WM_P'], concise=True, folder = FOLDER)
                VIEWER.display_gramWM_state(model.schemas['Grammatical_WM_P'], concise=True)
                VIEWER.display_lingWM_state(model.schemas['Semantic_WM_P'], model.schemas['Grammatical_WM_P'], concise=True)
...
#        model.schemas['Grammatical_WM_P'].show_state()
        if IMG_FILE:
            VIEWER.display_saccades(out_fixation, IMG_FILE, ss_radius=True)
