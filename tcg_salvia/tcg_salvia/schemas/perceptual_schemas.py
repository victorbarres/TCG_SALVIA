...
from ..neuro_st.schema_theory import KNOWLEDGE_SCHEMA, SCHEMA_INST, SYSTEM_SCHEMA, LTM, WM
...
#                if ss.area != self.focus_area and self.focus_area.includes(ss.area):
    ...
    def show_scene(self, img_file):
        ...
        if self.scene:
            ...
            VIEWER.display_scene(self.scene, img_file)
...
class SCENE_PERCEPTION(SYSTEM_SCHEMA):
    """
    Much simplified scene perception schema that retains the basic functionalities of SUBSCENE_RECOGNITION 
    but bypass the use of perceptual schemas and directly works with semantic representations.
    
    Data:
        - params: {'recognition_time':INT}: Defines the time it takes to perceive a sub_scene.
        - scene (SCENE_LIGHT): stores data based on scene data received as input (as defined by LOADER)
        - subscene (STR): Name of the currently perceived subscene.
        - uncertainty (INT): Remaining uncertainy in subscene perception.
        - next_saccade (BOOL): True ->  Triggers next saccade when the perception of the subscene is done. Else False.
...
#                self.scene.update_saliency(ss,1) # (Re)boosts the saliency of all the subscenes in focus.
    ...
    def get_state(self):
        """
        """
        data = super(SCENE_PERCEPTION, self).get_state()
        data['subscene'] = self.subscene
        data['uncertainty'] = self.uncertainty
        data['next_saccade'] = self.next_saccade
        return data
