...
class LOADER(object):
    ...
    def read_concept(atype, sup_cpt, cpt_knowledge, cpt_data):
        ...
        for concept in cpt_data:
            ...
            flag = LOADER.read_concept(atype, sub_cpt, cpt_knowledge, cpt_data[concept])
            if not(flag):
                return False
    ...
    def read_wk_frame(frame_knowledge, aFrame, cpt_knowledge):
        ...
        for node in aFrame['nodes']:
            LOADER.read_frame_node(new_frame, node, name_table, cpt_knowledge)
        for rel in aFrame['edges']:
            LOADER.read_rel(new_frame, rel, name_table, cpt_knowledge)
    ...
    def read_percept(atype, sup_per, per_knowledge, per_data):
        ...
        for percept in per_data:
            ...
            if(isinstance(per_data[percept], list)):
                ...
                flag = LOADER.read_percept('is_a', sub_per, per_knowledge, per_data[percept])
                if not(flag):
                    return False
    ...
    def read_semframe(new_cxn, SemFrame, name_table, cpt_knowledge):
        ...
        for node in SemFrame['nodes']:
            LOADER.read_node(new_cxn, node, name_table, cpt_knowledge)
        for rel in SemFrame['edges']:
            LOADER.read_rel(new_cxn, rel, name_table, cpt_knowledge)
    ...
    def read_synform(new_cxn, SynForm, name_table):
        ...
        for form_elem in SynForm:
            if form_elem['type'] == 'SLOT':
                LOADER.read_slot(new_cxn, form_elem, name_table)
            elif form_elem['type'] == 'PHON':
                LOADER.read_phon(new_cxn, form_elem, name_table)
    ...
    def read_cxn(grammar, aCxn, cpt_knowledge):
        ...
        # READ SEMFRAME
        LOADER.read_semframe(new_cxn, aCxn['SemFrame'], name_table, cpt_knowledge)
            
        # READ SYNFORM
        LOADER.read_synform(new_cxn, aCxn['SynForm'], name_table)
                
        # READ SYMLINKS
        LOADER.read_symlinks(new_cxn, aCxn['SymLinks'], name_table)
    ...
    def load_conceptual_knowledge(file_name='', file_path='./'):
        ...
        """
        # Open and read file
        json_data = LOADER.json_read(file_name, path=file_path)
        ...
        flag = LOADER.read_concept("is_a", top_cpt, my_conceptual_knowledge, cpt_data)
    ...
    def load_frame_knowledge(file_name='', file_path='./', cpt_knowledge = None):
        ...
        for frame in frame_data:
            wk_frame = LOADER.read_wk_frame(frame, cpt_knowledge)
    ...
    def load_perceptual_knowledge(file_name='', file_path='./'):
        ...
        flag = LOADER.read_percept('is_a', top_per, my_perceptual_knowledge, per_data)
        if not(flag):
            return None
    ...
    def load_grammar(file_name='', file_path='./', cpt_knowledge = None):
        ...
        for aCxn in gram_data:
            LOADER.read_cxn(my_grammar, aCxn, cpt_knowledge)
...
if __name__=='__main__':
    my_conceptual_knowledge = LOADER.load_conceptual_knowledge("TCG_semantics.json", "./data/semantics/")
    my_perceptual_knowledge = LOADER.load_perceptual_knowledge("TCG_semantics.json", "./data/semantics/")
    my_conceptualization = LOADER.load_conceptualization("TCG_semantics.json", "./data/semantics/", my_conceptual_knowledge, my_perceptual_knowledge)
    my_grammar = LOADER.load_grammar("TCG_grammar_VB.json", "./data/grammars/", my_conceptual_knowledge)
    my_scene = LOADER.load_scene("TCG_scene.json", "./data/scenes/TCG_cholitas/")
