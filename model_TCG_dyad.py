...
def test(seed=None):
    ...
    for t in range(max_time):
        if t > 10 and (
                t % speech_rate == 0) and lang_input:  # Need some time to have the system set up before it receives the first input.
            word_form = lang_input.pop()
            print('t: {}, VB says: {}'.format(t, word_form))
            language_system_1.set_input(word_form)

        if language_system_1.schemas[
            'Semantic_WM'].schema_insts and flag1:  # Switching from comprehension to production
        ...
        if output and output['Utter'] and flag2:
            ...
            print('t: {}, Agt1 says: {}'.format(t, output['Utter']))
            language_system_2.set_input(output['Utter'])

        if language_system_2.schemas[
            'Semantic_WM'].schema_insts and flag2:  # Switching from comprehension to production
        ...
        if output and output['Utter']:
            print('t: {}, Agt2 says: {}'.format(t, output['Utter']))

        language_system_1.update()
        language_system_2.update()

    language_system_1.schemas['Grammatical_WM_P'].show_dynamics(inst_act=True, WM_act=True, c2_levels=True,
                                                                c2_network=True)
    language_system_2.schemas['Grammatical_WM_P'].show_dynamics(inst_act=True, WM_act=True, c2_levels=True,
                                                                c2_network=True)
