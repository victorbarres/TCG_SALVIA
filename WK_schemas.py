    ...
    def process(self):
        ...
        if new_wk_frame_insts:
            ...
            for inst in new_wk_frame_insts:
                ...
                # Looking for possible competitors
                competitors = [old_inst for old_inst in self.schema_insts if
                               old_inst.content.name == inst.content.name and old_inst != inst]
        ...
        for wk_frame_inst in self.wk_frame_instances:
            for concept in concepts:
                if wk_frame_inst.has_trigger(concept):
                    ...
                    if self.verbose > 0:
                        print("{}: {} by {}".format(self.t, wk_frame_inst.content.name, concept.name))
            ...
            for concept in concepts:
                if wk_frame_schema.has_trigger(concept):
                    if not (new_instance):
                        ...
                        if self.verbose > 0:
                            print("{}: {} by {}".format(self.t, new_instance.content.name, concept.name))
                    else:
                        new_instance.got_trigger(concept)
                        if self.verbose > 0:
                            print("{}: {} by {}".format(self.t, new_instance.content.name, concept.name))
