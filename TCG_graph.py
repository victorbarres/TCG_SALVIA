...
#def find_sub_iso2(G, G_pat, node_match=None, edge_match=None): ### NEEDS TO RETURN ISOS THAT INCLUDE THE EDGE MAPPINGS!
#    """
#    Returns the list of all the graph isomorphisms between a subgraph of G and the graph pattern G_pat. Each isomorphim is defined as a dictionary with 
#    keys "nodes" itself a dictionary mapping G_pat nodes to G nodes, and "edges" a dictionary mappign G_pat edges to G edges.
#    
#    Args:
#        - G (NetworkX.Digraph)
...
#        iso = {"nodes":{}, "edges":{}}
#        for key in mapping:
#            iso["nodes"][mapping[key]] = {key: G.node[key]} # reverse mapping for convenience (SemFrame -> SemRep)        
...
## Find subgraph isomorphisms ###
def find_sub_iso(G, G_pat, node_match=None, edge_match=None, subgraph_filter=lambda x:True):
    """
    Returns the list of all the graph isomorphisms between a subgraph of G and the graph pattern G_pat. Each isomorphim is defined as a dictionary with 
    keys "nodes" itself a dictionary mapping G_pat nodes to G nodes, and "edges" a dictionary mappign G_pat edges to G edges.
