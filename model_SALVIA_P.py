...
def set_model(semantics_name='TCG_semantics_main', grammar_name='TCG_grammar_VB_main', verbal_guidance=False,
              model_params={}):
    """
    Sets up a SALVIA_P.
...
def set_inputs(model, input_name, input_file='TCG_scene.json', show_scene=True):
    ...
    SCENE_FOLDER = "{}{}/".format(SCENE_INPUT_PATH, input_name)
...
def run(model, sim_name, max_time=900, seed=None, verbose=0, prob_times=[], IMG_FILE=''):
    ...
    for t in range(max_time):
        ...
        if t in prob_times:
            TCG_VIEWER.display_WMs_state(model.schemas['Visual_WM'], model.schemas['Semantic_WM_P'],
                                         model.schemas['Grammatical_WM_P'], concise=True, folder=FOLDER)
