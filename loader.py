...
class TCG_LOADER(object):
    ...
    def json_read(file_name, path='./'):
        ...
        try:
            ...
            print("Cannot open {}. {}".format(file_name, e.strerror))
            return False

        try:
            json_data = json.load(f)
        except ValueError as e:
            print("Invalid json file {}. {}".format(file_name, e))
            return False
    ...
    def read_frame_node(new_frame, aNode, name_table, cpt_knowledge):
        ...
        name = aNode['name']
        new_node.name = '{}_{}'.format(name, new_node.id)

        concept = cpt_knowledge.find_meaning(aNode['concept'])
        if not concept:
            error_msg = "Cannot find concept {} in conceptual knowledge".format(aNode['concept'])
    ...
    def read_frame_rel(new_frame, aRel, name_table, cpt_knowledge):
        ...
        name = aRel['name']
        new_rel.name = '{}_{}'.format(name, new_rel.id)
    ...
    def read_wk_frame(aFrame, cpt_knowledge):
        ...
        new_frame.trigger = []
        for t in trigger:
            trigger_node = name_table['names'][t]
            trigger_node.trigger = True
            new_frame.trigger.append(name_table['names'][t])
    ...
    def read_slot(new_cxn, aSlot, name_table):
        ...
        name = aSlot['name']
        new_slot.name = '{}_{}'.format(name, new_slot.id)
    ...
    def read_phon(new_cxn, aPhon, name_table):  # REWORK THIS? SHOULD I RECONSIDER THE PHON TYPE?
        ...
        name = aPhon['name']
        new_phon.name = '{}_{}'.format(name, new_phon.id)
    ...
    def load_conceptualization(file_name='', file_path='./', cpt_knowledge=None, per_knowledge=None):
        ...
        for cpt in czer_data:
            if not (cpt_knowledge.has_concept(cpt)):
                print("{}: concept not found in sem_net".format(cpt))
            else:
                for pcpt in czer_data[cpt]:
                    if not (per_knowledge.has_percept(pcpt)):
                        print("{}: percept not found in perceptual knowledge".format(pcpt))
                    else:
    ...
    def load_scene(file_name='', file_path='./', percept_LTM=None):
        ...
        name_table = {}
        for i in [s for s in scene_input['schemas'].keys() if scene_input['schemas'][s][
                                                                  'type'] != 'RELATION']:  # First instantiate all the schemas that are not relations.
        ...
        for i in [s for s in scene_input['schemas'].keys() if
                  scene_input['schemas'][s]['type'] == 'RELATION']:  # Now dealing with relations
            dat = scene_input['schemas'][i]
            schema = [schema for schema in per_schemas if schema.content['percept'].name == dat['schema']][0]
            inst = ps.PERCEPT_SCHEMA_INST(schema, trace=schema)
            inst.content['pFrom'] = name_table[dat['from']]
            inst.content['pTo'] = name_table[dat['to']]
            area = ps.AREA(x=dat['location'][0], y=dat['location'][1], w=dat['size'][0],
                           h=dat['size'][1])  # This means that the area is gonna be of size 0
            area.set_BU_saliency(BU_saliency_map=None)  # THIS NEEDS TO BE CHANGED (for now random)
            inst.set_area(area)
            if dat['saliency'] == 'auto':
                inst.set_saliency(area.saliency)  # Does this make any sense at all??
    ...
    def load_scene_light(file_name='', file_path='./', scene_name='', concept_LTM=None):
        ...
        for ss_name, val in subscenes.iteritems():
            ...
            saliency = (BU_saliency.index(ss_name) + 1) / len(
                BU_saliency)  # Saliency is simply defined based on the index in the BU_saliency list (reversed)
    ...
    def load_sem_macro(macro_name, file_name='', file_path='./'):
        ...
        def substitute(my_input, a_substitution):
            pattern = lambda word: r'\b{}\b'.format(word)
            new_input = my_input
...
def test():
    ...
    my_frame_knowledge = TCG_LOADER.load_frame_knowledge("TCG_semantics_dev.json", "./data/semantics/",
